...
import com.guanchedata.infrastructure.adapters.apiservices.ingestbookservice.IngestBookService;
...
public class Main {
    public static void main(String[] args) {
        ...
        GutenbergBookProvider gutenbergBookProvider =
                new GutenbergBookProvider(new GutenbergFetch(), new GutenbergConnection(),
                        new GutenbergBookContentSeparator());

        ActiveMQBookIngestedNotifier notifier =
                new ActiveMQBookIngestedNotifier(System.getenv("BROKER_URL"));
        HazelcastManager hazelcastManager =
                new HazelcastManager("SearchEngine",
                        Integer.parseInt(System.getenv("REPLICATION_FACTOR")), gutenbergBookProvider,
                        storageDate);
        BookDownloadStatusStore bookDownloadLog =
                new BookDownloadLog(hazelcastManager.getHazelcastInstance(), "log");
        BookDownloader ingestBookService =
                new IngestBookService(new IngestBookInfrastructure(storageDate, bookDownloadLog, notifier,
                        hazelcastManager, gutenbergBookProvider));
